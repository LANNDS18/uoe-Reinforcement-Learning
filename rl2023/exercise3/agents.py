    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(timestep: int, max_timestep: int):
            exploration_timesteps = int(self.exploration_fraction * max_timestep)
            return max(self.epsilon_min, self.epsilon_start - (self.epsilon_start - self.epsilon_min) * (
                    timestep / exploration_timesteps))

        def epsilon_exponential_decay(timestep: int, max_timestep: int):
            if timestep == 0:
                return self.epsilon_start
            r = self.epsilon_exponential_decay_factor ** (1 / max_timestep)
            eps = self.epsilon_start * (r ** timestep)
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        actions = actions.clone().detach().to(torch.int64)

        # compute target Q values
        with torch.no_grad():
            q_next = self.critics_target.forward(next_states)
            max_next_q_values = q_next.max(dim=1, keepdim=True).values
            target_q_values = rewards + self.gamma * max_next_q_values * (1 - dones)

        q_values = self.critics_net(states).gather(1, actions)
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        for t in reversed(range(T - 1)):
            # print(f"T: {T}, t: {t}, t+1: {t+1}")
            G = rewards[t + 1] + self.gamma * G
            obs = torch.from_numpy(observations[t]).float()
            probs = self.policy(obs)
            m = Categorical(probs)
            log_prob = m.log_prob(actions[t])
            p_loss = p_loss - G * log_prob
