        ...
        while delta >= theta:
            ...
            for s in range(self.state_dim):
                ...
                for a in range(self.action_dim):
                    A[a] = np.sum(mdp.P[s, a] * (mdp.R[s, a] + self.gamma * V))
                V[s] = np.amax(A)
                delta = np.amax([delta, np.abs(v - V[s])])
    ...
    def _policy_eval(self, policy: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while delta >= self.theta:
            ...
            for s in range(self.state_dim):
                v = V[s]
                V[s] = np.sum(mdp.P[s, np.argmax(policy[s])] * (mdp.R[s, np.argmax(policy[s])] + self.gamma * V))
                delta = np.amax([delta, np.abs(v - V[s])])
        return np.array(V)
